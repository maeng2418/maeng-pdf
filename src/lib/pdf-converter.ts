...
export class PDFConverter {
  static async imagesToPDF(files: File[], options: ImageToPdfOptions = {}): Promise<Blob> {
    ...
    if (mergeIntoOne) {
      return this.createSinglePDFFromImages(files, { quality, pageSize, orientation, password: options.password });
    } else {
      return this.createMultiplePDFsFromImages(files, { quality, pageSize, orientation, password: options.password });
    }
  }
  ...
  private static async createSinglePDFFromImages(
    ...
    if (options.password) {
      return this.encryptPDF(pdfBlob, options.password);
    }
  ...
  static async wordToPDF(file: File, options: ConversionOptions = {}): Promise<Blob> {
    try {
  ...
  static async excelToPDF(file: File, options: ConversionOptions = {}): Promise<Blob> {
    try {
  ...
  private static async encryptPDF(pdfBlob: Blob, password: string): Promise<Blob> {
    try {
      ...
      return new Blob([new Uint8Array(encryptedPdfBytes)], { type: 'application/pdf' });
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : '알 수 없는 오류';
      throw new Error(`PDF 암호화 중 오류가 발생했습니다: ${errorMessage}`);
    }
  }
}